...
| **Audit-mode view-key export (2026-05-22)** | ✅ ADR-011 "View key audit opt-in" satırı uygulandı. `qv-wallet::view_export::ViewKeyExport` JSON formatı (`.qvview`) — hibrit view keypair + Dilithium spend public key (spend secret YOK). CLI: `qv-wallet export-view-key --out alice.qvview --account 0 [--label "Q2 audit"]`. Auditör tarafı: `qv-wallet audit-scan --view-key alice.qvview --rpc <url>` → keystore'a değmeden `qv_scanStealth`'i tetikler ve gelen ödemeleri listeler. `RpcClient::scan_stealth_with_view_key(view_kp, spend_pk, ...)` — spend secret'a hiç ihtiyaç duymayan ayrı method. 4 birim test (export/load roundtrip, encap/decap fonksiyonel doğrulama, format reddi, geçersiz Kyber seviye reddi) |
| **Encrypted address book (2026-05-22)** | ✅ `qv-wallet::address_book` — etiketli stealth-adres kişileri (`alice → qvst1…`). Argon2id+AES-256-GCM ile şifreli, keystore'un yanına `wallet.json.contacts` olarak; aynı parolayla açılır. CLI: `qv-wallet contacts {add,list,remove,show}`. `send-stealth --to-contact alice` ile etiket üzerinden gönderim. HTTP API: `GET/POST /api/contacts`, `POST /api/contacts/remove`. UI'da "Address book" paneli (etiket+adres+not + ekle/sil) ve Send formunda kişi dropdown'ı. Add işlemi adresi doğrular (parse edilemezse reddedilir), duplicate label reddedilir. 7 birim test (CRUD + invalid input + encrypted save/load + wrong password reject + distinct ciphertexts + empty-file load). UnlockedWallet artık keystore parolasını da kilitliyken RAM'de tutuyor — kilit açma dışında parola tekrar sorulmuyor |
| **Per-output selective disclosure (2026-05-22)** | ✅ ADR-011 "Selective disclosure opt-in" satırı uygulandı. `qv-wallet::disclose::DisclosureFile` JSON formatı (`.qvdisclose`) tam self-contained: outpoint + on-chain stealth bayt verisi + spend public key + DisclosureProof bağlantı karma'sı. CLI: `qv-wallet disclose --utxo <tx:idx> --out f.qvdisclose [--amount N --label "..."]` ve `qv-wallet verify-disclosure --proof f.qvdisclose` — verifier'ın keystore'a, RPC'ye veya cüzdan kuruluşuna ihtiyacı YOK. 4 birim test (full owner→discloser→auditor roundtrip miktarlı/miktarsız, tampering reddi, format reddi). `StealthScan` wire format `kem_ciphertext_hex` + `view_tag_hex` + `kyber_level` ile zenginleştirildi (tek RPC çağrısı yeterli). **Cüzdan UI'da**: adres panelinde "Download view key (audit)" butonu; UTXO tablosunda stealth satırları için "Disclose" butonu → form (amount + label) → tarayıcı `.qvdisclose` dosyasını indirir |
| **Multi-account UI + Tx history + Devnet faucet + QR scanner (2026-06-02)** | ✅ Dört cüzdan UX iyileştirmesi tek seansta uygulandı. **(1) Multi-account UI**: `GET /api/wallet/accounts` keystore `view_keypairs`'i dolaşarak her hesabın stealth adresini + fingerprint'ini döndürür; `POST /api/wallet/switch-account` `unlock_account`'i çağırarak yeni hesabın view key'ini ilk seferde üretip persist eder, mevcut UnlockedWallet'i değiştirir. Address panelinde dropdown + "+ New" butonu. **(2) Transaction history**: `qv-wallet::history` modülü — `HistoryLog` (Sent entry'ler) + `merge_with_received` (live UTXO scan'inden Received entry'ler türetir). Argon2id+AES-256-GCM ile `wallet.json.history`'ye persist. `handle_send` başarılı broadcast sonrası `record_send` çağırır (alıcı fingerprint + kişi defterinden etiket). UI'da filtreli (`All/Sent/Received/This account`) tablo paneli. 8 birim test (append+iter, encrypted roundtrip, wrong password, empty file, distinct ciphertexts, record_send persist, merge sort, serde tag). **(3) Devnet faucet**: `qv-wallet::faucet::drip` — `DEVNET_TEST_MNEMONIC`'in 0. hesabından deterministik spend key türetir, `qv_scanP2pkh(devnet_pk_hash)` ile UTXO toplar, greedy seçim + stealth output (alıcı = kullanıcının cüzdanı) + p2pkh_pqc change (devnet hash'ine geri) inşa eder, broadcast eder. `POST /api/devnet/faucet`. UI'da Balance panelinde "Get devnet test funds" butonu + miktar/fee formu. 3 birim test (deterministic key derivation, zero amount reddi, overflow reddi). **(4) QR scanner (kamera)**: Send formunda "Scan QR with camera" butonu; modal panel `BarcodeDetector` API'siyle `QVADDR1:k/N:HEX` parçalarını birikiminde toplar, son parça gelince `/api/wallet/qr-reassemble` ile recombine eder ve `send-to`'yu doldurur. `getUserMedia({facingMode:'environment'})` ile arka kamera tercih edilir. `BarcodeDetector` yoksa açık not |
| **D-1: ReadInputDatum opcode — AMM/lending kovenant'larının önkoşulu (2026-06-05)** | ✅ `OpCode::ReadInputDatum (0x6A)` `0x60-0x6F` introspection bloğuna eklendi. Stack: pop index `i` → push resolved input #i'nin datum baytları (datum yoksa boş Vec). `ReadOutputDatum`'a paralel. Interpreter implementasyonu `ctx.resolved_inputs[i].datum` erişiyor, out-of-range index'te `IndexOutOfRange` hatası. Gas tablosu 10 birim (diğer introspection opcode'larıyla aynı). `from_byte` + `Display` arm'ları 0x6A ↔ "READ_INPUT_DATUM". 3 unit test: datum yokken boş bytes, attached datum'lardan bayt-bayt doğru push, out-of-range index reddi. `all_opcodes_have_mnemonics` testine de satır eklendi. D-2 (AMM kovenant locking script) için temel yetenek hazır; OTURUM-DEVAM-PROMPTU.md'de D-2'nin opcode-by-opcode plan'ı + 3 açık tasarım kararı (self-script-hash döngüsü, u128 çarpım, u64 LE bayt → integer) işli |
| **Binary dağıtımı: release workflow + sürüm damgası + kurulum/operatör kılavuzları (2026-06-05)** | ✅ Non-developer dağıtım altyapısı tamamlandı. **(1) GitHub Actions release pipeline** (`.github/workflows/release.yml`): tag push'ta (`v*.*.*`) Windows x64 (`x86_64-pc-windows-msvc`) ve macOS arm64 (`aarch64-apple-darwin`) runner'larında `qv-wallet`/`qv-node`/`qv-miner` üçü `--release` ile derlenir; zip (Windows) ya da tar.gz (macOS); sha256 manifest'i + `BUILD-INFO.txt` (tag + git_sha + timestamp). `softprops/action-gh-release@v2` ile Release'e attach. RC/beta/alpha tag'leri otomatik prerelease işaretlenir. Manuel tetik desteği. **(2) Sürüm damgası**: üç binary crate'te `build.rs` (60 satır) — `QV_BUILD_GIT_HASH` env (CI) ya da `git rev-parse --short=12 HEAD` (lokal) → `QV_GIT_HASH`/`QV_BUILD_TAG`/`QV_BUILT_AT` rustc-env'lerine yazılır. `cli.rs`'lerde `VERSION_STRING` const'u `#[command(version = ...)]` ile clap'a bağlandı. Çıktı: `qv-wallet 0.1.0 (v0.1.0, git a1b2c3d4e5f6)`. **(3) `docs/INSTALL.md`** — Windows + macOS kullanıcı kılavuzu: indirme + sha256 doğrulama + Defender exclusion / Gatekeeper quarantine + ilk cüzdan + public devnet'e bağlanma + faucet + self-hosted node + troubleshooting tablosu. **(4) `docs/PUBLIC-DEVNET-RPC.md`** — sunucu operatörü kılavuzu: systemd sertleştirilmiş unit (`NoNewPrivileges`, `ProtectSystem=strict`, `MemoryDenyWriteExecute`), Caddy reverse proxy + Let's Encrypt + `caddy-ratelimit` (xcaddy build), fail2ban anti-abuse, Prometheus monitoring, logrotate, upgrade flow. **(5) `--network devnet`/`local` alias'ı**: `PUBLIC_DEVNET_RPC_URL`/`LOCAL_DEVNET_RPC_URL` sabit, `Cli::effective_rpc_url()`, INSTALL.md kısa yol kullanır. ROADMAP "Faz 9 — Production hardening" Linux x64 hedefi eklenmesi + Apple Developer imza için takip notu açıldı |
| **Multi-tenant cüzdan + LAN bind + monitör tx geçmişi + lag-watchdog (2026-06-03)** | ✅ Beş iş tek seansta. **(1) Multi-tenant cüzdan (Seçenek B custodial demo).** Yeni `qv-wallet::session` modülü — TTL'li `SessionStore<token, SessionEntry>`, kullanıcı adı validasyonu (a-z0-9_-, 3-32 char, path traversal koruması), `user_keystore_path` layout. `AppState` tek `Mutex<Option<UnlockedWallet>>` yerine `Backend::{Single, Multi}` enum'una refactor edildi; `Bearer(Option<String>)` axum extractor token'ı `Authorization: Bearer` header'ından alıyor. Yeni route'lar `/api/auth/{register,login,logout,me}`; mevcut tüm handler'lar `state.require_active(&bearer)` üzerinden session-scoped çalışıyor. CLI `serve --wallets-dir <path> --session-ttl-secs N` ile aktive edilir. UI: register/login paneli, localStorage token, "Lock"→"Logout" dinamik label. 14 yeni unit test (session store + username validation). **(2) LAN bind.** `--wallets-dir` set'liyse `0.0.0.0:7777` izinli (multi-tenant + token auth ile koruma); single-user mode'da hâlâ loopback zorunlu. **(3) HTTP'de clipboard + QR auth düzeltmesi.** `navigator.clipboard` sadece HTTPS/localhost'ta çalıştığı için `copyText()` `document.execCommand('copy')` fallback'i ile gelir; `<img src>` token taşımadığından `loadFingerprintQr()` `authFetch` + blob URL ile çevrildi; `.qvaddr` / `.qvview` indirme `authFetch` üzerinden. **(4) node-monitor tx geçmişi büyütüldü.** `recordBlockTxs()` taranan bloklardaki her tx'i kalıcı geçmişe yazıyor (mempool poll'u kaçırılan hızlı tx'leri yakalar). Varsayılan blok penceresi 60→600 (~5 dk @ 0.5sn slot), max 300→5000, `txHistoryMap` cap 600→10000, döndürülen geçmiş 300→2000. UI'a "300/600/1200/2400/5000 blok" buton seçici eklendi. **(5) Lag-restart watchdog (Seçenek B).** `devnet/watchdog.ps1` — `nodeN.cmd` JSON metadata'sından her node'un launch komutunu okur, 30sn'de bir `qv_getTip` ile cluster max tip'ini hesaplar, ardışık 2 kontrol >1000 blok geride kalan node'u `Stop-Process` + aynı argümanlarla yeniden başlatır, `pids` dosyasını günceller. `run-devnet.ps1` `QV_LAG_RESTART` env'i set'liyse watchdog'u arka plana atar. Memory storage backend ile restart = peer'lerden ADR-010 SyncManager üzerinden full re-sync |
...
| ~~K-05~~ | ✅ **KAPATILDI 2026-05-22** — Yeni `qv_getPostApplyCommitment(tx_bytes_hex: Vec<String>) -> hex(32B)` RPC eklendi. Node tarafında `qv-node::slot_ticker::speculative_utxo_commitment` (in-process path ile aynı helper) çağrılır → snapshot + speculative apply → root. Miner `main::cmd_run`'taki block_producer_fn closure'u artık `pending_hex`'i bu RPC'ye geçirip dönüşü header'a stamp ediyor. `produce_block` / `produce_block_with_decryption` reference helper'ları ZERO basmaya devam ediyor (test ergonomy + verifier alanı opaque). `producer_key_hash = ZERO` placeholder'ı miner main.rs'inde zaten gerçek (`sha3(pool_id)`) — reference helper'ında hâlâ ZERO ama production yol gerçek | — |
...
| ID | Yer | Sorun | Kapatan faz |
|---|---|---|---|
| ~~MP-01~~ (kısmen) | ✅ **WIRING KAPATILDI 2026-05-22** — `qv-mempool::DkgEnvelopeDecryptor` artık `qv-crypto::threshold::DkgThresholdDecryptor`'a bağlı. Hibrit zarf: ElGamal-style DKG ile 32-byte AES anahtarı sarılır, AES-256-GCM ile tx body. `encrypt_envelope`, `encrypt_envelope_random`, `create_envelope_share` API'leri eklendi. 5 shape/error-path test'i yeşil. Tam DKG → encrypt → share → combine → decrypt uçtan uca testi (`envelope_full_roundtrip_with_real_dkg`) `#[ignore]` — **T-01 (Pedersen DKG / Feldman VSS doğrulama)** kapanınca yeşillenir. `MockThresholdDecryptor` geri uyumluluk için duruyor | Faz 7 (T-01 ile birlikte tamamen kapanır) |
